...
describe('RangeCalculator', () => {
  ...
  test('current Params and range is set on initialisation', async() => { 
    ...
    expect(currentParams).toEqual({ speed: 70, temp: 40, ac: 'off', wheelsize: 19 })
    expect(document.getElementById('100d-range').getAttribute('range')).toBe('853');
    expect(document.getElementById('p100d-range').getAttribute('range')).toBe('822');
  });
  ...
  test('range updated when speed is changed', () => {
    ...
    expect(currentParams).toEqual({ speed: 80, temp: 40, ac: 'off', wheelsize: 19 })
    expect(document.getElementById('100d-range').getAttribute('range')).toBe('763');
    expect(document.getElementById('p100d-range').getAttribute('range')).toBe('736');
  });
  ...
  test('range updated when temperature is lowered, ac is on and wheel is bigger', () => {
    ...
    expect(currentParams).toEqual({ speed: 70, temp: 30, ac: 'on', wheelsize: 21 })
    expect(document.getElementById('100d-range').getAttribute('range')).toBe('751');
    expect(document.getElementById('p100d-range').getAttribute('range')).toBe('693');
  });
